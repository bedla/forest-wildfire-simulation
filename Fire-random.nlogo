...
to go
  if not any? fires  ;; either fires or embers
  [
    stop
  ]
  ...
  [    
    ...
    if first?
    [
      set this-parent self      
    ]
    
    if not first?
    [
      set x [ xcor ] of parent
      set y [ ycor ] of parent
    ]
    
    ask neighbors4 with [ shade-of? pcolor green ]
    [
      if distancexy x y < fire-radius
      [
        ignite this-parent false
      ]
    ]
...
to reburn-fire
  let possible-fires fires with [ first? ]
  if any? possible-fires
  [
    ask one-of possible-fires
    [
      let this-fires (fires-in-radius xcor ycor fire-radius) with [ not first? ]
      if not any? this-fires
      [
        ask patches in-radius fire-radius
        [
...
to-report fires-in-radius [x y radius]
  let result []
  ask patch x y
  [
    ask patches in-radius radius
    [      
      set result lput fires-here result
    ]
  ]
...
;; patch procedure
to ignite [ value-parent value-first? ]
  sprout-fires 1
  [
    set color red
...
In other publications, please use:  Copyright 1997 Uri Wilensky.  All rights reserved.  See http://ccl.northwestern.edu/netlogo/models/Fire for terms of use.
@#$#@#$#@
default
true
0
