...
to-report mean-value-from-list [myList default precisionStep]
  if length myList <= 0
  [
    report default
  ]
  ...
  while [ result = nobody ]
  [
    let l filter [ ? >= numberL and ? <= numberR ] myList
    ...
    [
      set numberL numberL - precisionStep
      set numberR numberR + precisionStep
    ]
  ]
...
to-report best-canyon-patch [thisPatch radius last-el]
  let result nobody
  ask thisPatch
  [
    let thisElevation elevation
    let diffList []
    ask patches in-radius radius
;    ask neighbors
    [
      let diff thisElevation - elevation
      if diff <= 0
      [
        set diffList lput elevation diffList
        set diffList remove last-el diffList
      ]
    ]
    ...
    [    
      let number mean-value-from-list diffList -1 10
;      print (word "prum=" mean diffList " n=" number " " diffList)
;      set result patches in-radius radius with [ elevation = min diffList ]
      set result patches in-radius radius with [ elevation = number ]
;      set result neighbors with [ elevation = min diffList ]
    ]
  ]  
  
  if is-agentset? result
  [
...
to flow ;; turtle procedure
  ...
  let last-el last-elevation
  set last-elevation elevation
  let target best-canyon-patch self 10 last-el
  if target != nobody
  [
    move-to target
  ]
...
;  let target min-one-of neighbors [ thisElevation - elevation ]
;  move-to target
